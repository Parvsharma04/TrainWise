...
while True:
    try:
        ...
        if frame_data:
            ...
            except (cv2.error, ValueError, TypeError) as decode_error:
                print(json.dumps({"error": f"Decoding/Processing Error: {decode_error}"}))
                sys.stdout.flush()
            except Exception as e:
                print(json.dumps({"error": f"General Processing Error: {e}"}))
                sys.stdout.flush()
